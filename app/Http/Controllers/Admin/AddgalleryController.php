...
class AddgalleryController extends Controller
{
...

    {
        $language = DB::table('avig_language')
					...
					->orderBy('id','asc')
					->get();
        return view('admin.addgallery', ['language' => $language]);
    ...
    protected function addgallerydata(Request $request)
    {
	   ...
	   $settings = DB::select('select * from settings where id = ?',[1]);
		...
		$idd = DB::table('gallery')-> insertGetId(array(
        'name' => $pagenamo,
        'lang_code' => $code,
		'token' => $token,
		'parent' => $parent,

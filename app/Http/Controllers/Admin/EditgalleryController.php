...
class EditgalleryController extends Controller
{
	...
	$language = DB::table('avig_language')
      ...
      return view('admin.editgallery',['gallery'=>$gallery, 'language' => $language]);
   }
    ...
    protected function editgallerydata(Request $request)
    {
        /*return User::create([
            'name' => $data['name'],
            'email' => $data['email'],
		...
		foreach($data['code'] as $index => $code)
		{
		   ...
		   if($code=="en")
			{
			  DB::update('update gallery set name="'.$pagename.'",lang_code="'.$code.'" where id = ?', [$id]);
			}
			else
			{
			    $counts = DB::table('gallery')
			     ...
			     if($counts==0)
				 {
				     ...
				     DB::insert('insert into gallery (name, lang_code, token, parent) values (?, ?, ?, ?)', [$pagenamo,$code,$token,$id]);
				 }
				 else
				 {
				   DB::update('update gallery set name="'.$pagename.'" where lang_code="'.$code.'" and parent = ?', [$id]);
				 }

...
$default_cnt = DB::table('avig_language')
	              ->where('lang_default','=',1)
		           ->count();
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }//changed
		$blogs = DB::table('post')
		         ->where('post_status', '=', '1')
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	...
	$data = array('post' => $post, 'post_count' => $post_count, 'previous' => $previous, 'previous_link' => $previous_link, 'next_link' => $next_link, 'next' => $next, 'popular_blog' => $popular_blog, 'gets' => $gets);
	 return view('blog')->with($data);

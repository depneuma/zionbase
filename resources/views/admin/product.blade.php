        ...
        <div class="right_col" role="main">
		 ...
		 <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
				  ...
				  <?php } else { ?>
				  <a href="<?php echo $url;?>/admin/add-product" class="btn btn-primary">Add Product</a>
				  <?php } ?>
                  <div class="x_content">
          ...
          <span class="sub"></span> Select All </button></th>
					  ...
					  foreach ($products as $product) { ?>
                        ...
                        <tr>
                         <td><input type="checkbox" name="prod_token[]" value="<?php echo $product->prod_token;?>"/></td>
                        
						 <td><?php echo $i;?></td>
						 <?php 
						 
						 $view_count = DB::table('product_images')
		                               ->where('prod_token', '=', $product->prod_token)
									   ->count();
						 if(!empty($view_count))
						 {
						 $view_image = DB::table('product_images')
		                               ->where('prod_token', '=', $product->prod_token)
									   ->get();
						 
					   $media="/media/";
						$path ='/local/images'.$media.$view_image[0]->image;
						if($view_image[0]->image!=""){
						?>
                         ...
                         <?php } ?>
                          ...
                          <td><?php if(!empty($product->prod_offer_price)){?><span style="text-decoration:line-through; color:#FF0000;"><?php echo $setts[0]->site_currency.' '.number_format($product->prod_price,2);?></span> <?php echo $setts[0]->site_currency.' '.number_format($product->prod_offer_price,2);?> <?php } else { ?> <?php echo $setts[0]->site_currency.' '.number_format($product->prod_price,2);?> <?php } ?></td>
                          
						  <?php if(empty($product->prod_available_qty)) { $valueqty = "Out of stock"; $cc="red"; } else  { $valueqty = $product->prod_available_qty; $cc=""; } ?>
                          <td style="color:<?php echo $cc;?>">
						  
						  <?php echo $valueqty;?></td>
				  ...
				  <?php } else { ?>
						  ...
						  <a href="<?php echo $url;?>/admin/edit-product/{{ $product->prod_id }}" class="btn btn-success">Edit</a>
						  <?php } ?>
				   <?php if(config('global.demosite')=="yes"){?>
